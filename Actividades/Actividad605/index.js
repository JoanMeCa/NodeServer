...
app.get('/pedido/:id', (req, res) => {
    ...
    const usuarios = db.prepare('SELECT ID, nombre FROM usuarios').all();
    const productos = db.prepare('SELECT id, nombre FROM productos').all();

    res.render('pedidodetalle', { pedido: pedido, usuarios: usuarios, productos: productos });
});
...
app.post('/actualizar-usuario', (req, res) => {
    const { id, nombre, email, action } = req.body;
    if (action === 'guardarSalir') {
        const update = db.prepare('UPDATE usuarios SET nombre = ?, email = ? WHERE ID = ?');
...
app.post('/actualizar-producto', (req, res) => {
    const { id, nombre, precio, action } = req.body;
    if (action === 'guardarSalir') {
        const update = db.prepare('UPDATE productos SET nombre = ?, precio = ? WHERE id = ?');
        update.run(nombre, precio, id);
        res.redirect('/productos');
    } else {
        res.redirect('/productos');
    }
...
app.post('/actualizar-pedido', (req, res) => {
    console.log("Ejecuta actualizar")
    const { id, usuario, producto, action } = req.body;
    console.log("ID")
    console.log(id)
    console.log("usuario")
    console.log(usuario)
    ...
    console.log(action)
    if (action === 'guardarSalir') {
        const update = db.prepare('UPDATE pedidos SET id_usuario = ?, id_producto = ? WHERE ID = ?');
        update.run(usuario, producto, id);
        res.redirect('/pedidos');
    } else {
        res.redirect('/pedidos');
    }
